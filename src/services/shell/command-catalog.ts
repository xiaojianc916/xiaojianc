...
const getPrimaryName = (entry: Pick<IShellCommandNodeSpec, 'names'>): string =>
  entry.names[0] ?? '';
    ...
    indexStatePromise = (async () => {
      const indexLoader =
        Object.entries(shellCatalogIndexModules).find(([modulePath]) =>
          modulePath.endsWith('/index.json'),
        )?.[1] ?? null;
...
export const loadShellCommandSpec = async (name: string): Promise<IShellCommandNodeSpec | null> => {
  ...
  const specPromise = (async () => {
    ...
    const localSpec =
      ...
      (generatedEntry
        ? (localRootEntriesByAnyName.get(toLookupKey(generatedEntry.label)) ?? null)
        : null);
