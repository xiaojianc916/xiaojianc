...
const TOOL_KIND_MATCHERS: readonly IToolKindMatcher[] = [
  ...
  {
    kind: 'git',
    patterns: ['get_changed_files', 'github_repo', 'git_'],
  },
  {
    kind: 'browser',
    patterns: [
  ...
  {
    kind: 'memory',
    patterns: ['^memory$', 'resolve_memory_file_uri'],
  },
  {
    kind: 'thinking',
    patterns: ['sequentialthinking', 'thinking', 'reason'],
  },
].map((item) => ({
