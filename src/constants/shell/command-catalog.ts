...
const sharesSpecName = (leftEntry: TNamedCatalogSpec, rightEntry: TNamedCatalogSpec): boolean => {
...
const synthesizeValueArgument = (flagLabel: string): IShellCommandArgumentSpec => ({
  label: 'value',
  detail: `${flagLabel} 参数`,
});
...
const flag = (label: string, detail: string, config: TFlagConfig = {}): IShellCommandOptionSpec => {
  const { aliases, takesValue, arg, args, ...restConfig } = config;
  const resolvedArgs = args ?? (takesValue ? [synthesizeValueArgument(label)] : undefined);
...
const mergeNamedList = <T extends TNamedCatalogSpec>(
  ...
  for (const overrideEntry of overrideList) {
    ...
    mergedList[matchedIndex] = mergeEntry(mergedList[matchedIndex], overrideEntry);
  }
...
): IShellCommandOptionSpec[] => mergeNamedList(baseOptionList, overrideOptionList, mergeOptionSpec);
      ...
      command('restore', '恢复工作区或暂存区文件', {
        flags: [
          ...
          flag('--source', '指定恢复来源', {
            takesValue: true,
            args: [
              argument('treeish', '恢复来源引用', { suggestions: GIT_RESTORE_SOURCE_SUGGESTIONS }),
            ],
          }),
      ...
      command('build', '构建镜像', {
        flags: [
          ...
          flag('--platform', '指定构建目标平台', {
            takesValue: true,
            args: [
              argument('platform', 'Docker 目标平台', { suggestions: DOCKER_PLATFORM_SUGGESTIONS }),
            ],
          }),
          ...
          command('up', '启动服务', {
            flags: [
              ...
              flag('--scale', '调整服务副本数', {
                takesValue: true,
                args: [
                  argument('service=count', '服务扩容表达式', {
                    suggestions: DOCKER_SCALE_SUGGESTIONS,
                  }),
                ],
              }),
            ],
          ...
          command('down', '停止并移除服务', {
            flags: [
              ...
              flag('--rmi', '同时移除镜像', {
                takesValue: true,
                args: [
                  argument('mode', '镜像移除范围', {
                    suggestions: DOCKER_IMAGE_PRUNE_MODE_SUGGESTIONS,
                  }),
                ],
              }),
            ],
          ...
          command('ps', '查看服务容器状态', {
            flags: [
              ...
              flag('--format', '指定输出格式', {
                takesValue: true,
                args: [
                  argument('format', 'Compose 状态输出格式', {
                    suggestions: DOCKER_FORMAT_SUGGESTIONS,
                  }),
                ],
              }),
              flag('--status', '按状态过滤', {
                takesValue: true,
                args: [
                  argument('status', 'Compose 容器状态', {
                    suggestions: DOCKER_STATUS_SUGGESTIONS,
                  }),
                ],
              }),
            ],
      ...
      command('image', '管理镜像对象', {
        subcommands: [
          ...
          command('prune', '清理未使用镜像', {
            flags: [flag('-a', '移除所有未使用镜像'), flag('-f', '跳过确认')],
          }),
          command('tag', '为镜像打标签'),
        ],
      }),
          ...
          command('create', '创建网络', {
            flags: [
              flag('--driver', '指定网络驱动', {
                takesValue: true,
                args: [
                  argument('driver', 'Docker 网络驱动', {
                    suggestions: DOCKER_NETWORK_DRIVER_SUGGESTIONS,
                  }),
                ],
              }),
      ...
      command('ps', '列出容器', {
        flags: [
          ...
          flag('--format', '指定输出格式', {
            takesValue: true,
            args: [
              argument('format', '容器列表输出格式', { suggestions: DOCKER_FORMAT_SUGGESTIONS }),
            ],
          }),
      ...
      command('run', '创建并运行容器', {
        flags: [
          ...
          flag('-p', '端口映射', {
            takesValue: true,
            args: [
              argument('mapping', '端口映射表达式', {
                suggestions: DOCKER_PORT_MAPPING_SUGGESTIONS,
              }),
            ],
          }),
          flag('-e', '设置环境变量', { takesValue: true }),
          flag('-v', '挂载卷', {
            takesValue: true,
            args: [
              argument('volume', '卷挂载表达式', {
                suggestions: DOCKER_VOLUME_MAPPING_SUGGESTIONS,
              }),
            ],
          }),
          ...
          flag('--network', '指定网络', {
            takesValue: true,
            args: [
              argument('network', '容器网络模式', { suggestions: DOCKER_RUN_NETWORK_SUGGESTIONS }),
            ],
          }),
          flag('-it', '分配交互式 TTY'),
        ],
  ...
  command('kubectl', 'Kubernetes 集群管理命令', {
    ...
    subcommands: [
      ...
      command('expose', '为资源创建 Service', {
        flags: [
          ...
          flag('--type', 'Service 类型', {
            takesValue: true,
            args: [
              argument('type', 'Kubernetes Service 类型', {
                suggestions: KUBECTL_SERVICE_TYPE_SUGGESTIONS,
              }),
            ],
          }),
        ],
      ...
      command('top', '查看资源监控指标', {
        subcommands: [command('pod', '查看 Pod 资源使用'), command('node', '查看 Node 资源使用')],
      }),
      command('wait', '等待资源达到指定状态', {
        flags: [
          flag('--for', '指定等待条件', {
            takesValue: true,
            args: [
              argument('condition', '等待条件', {
                suggestions: KUBECTL_WAIT_CONDITION_SUGGESTIONS,
              }),
            ],
          }),
          flag('--timeout', '设置超时时间', {
            takesValue: true,
            args: [
              argument('duration', '等待超时时间', { suggestions: KUBECTL_TIMEOUT_SUGGESTIONS }),
            ],
          }),
        ],
        subcommands: KUBERNETES_RESOURCE_COMMANDS,
  ...
  command('systemctl', 'systemd 服务管理命令', {
    flags: [
      ...
      flag('-o', '指定输出格式', {
        ...
        takesValue: true,
        args: [
          argument('format', 'systemctl 输出格式', { suggestions: SYSTEMD_OUTPUT_SUGGESTIONS }),
        ],
      }),
    ],
    subcommands: [
      ...
      command('list-units', '列出当前单元', {
        flags: [
          flag('--type', '按单元类型过滤', {
            takesValue: true,
            args: [
              argument('type', 'systemd 单元类型', { suggestions: SYSTEMD_UNIT_TYPE_SUGGESTIONS }),
            ],
          }),
          flag('--state', '按状态过滤', {
            takesValue: true,
            args: [
              argument('state', 'systemd 单元状态', {
                suggestions: SYSTEMD_UNIT_STATE_SUGGESTIONS,
              }),
            ],
          }),
          flag('--all', '显示全部单元'),
      ...
      command('list-unit-files', '列出已安装单元文件', {
        flags: [
          ...
          flag('--state', '按状态过滤', {
            takesValue: true,
            args: [
              argument('state', 'systemd 单元文件状态', {
                suggestions: SYSTEMD_UNIT_FILE_STATE_SUGGESTIONS,
              }),
            ],
          }),
        ],
  ...
  command('ssh', '建立 SSH 远端连接', {
    flags: [
      ...
      flag('-L', '本地端口转发', {
        takesValue: true,
        args: [
          argument('forward', '本地端口转发表达式', { suggestions: SSH_LOCAL_FORWARD_SUGGESTIONS }),
        ],
      }),
      flag('-R', '远端端口转发', {
        takesValue: true,
        args: [
          argument('forward', '远端端口转发表达式', {
            suggestions: SSH_REMOTE_FORWARD_SUGGESTIONS,
          }),
        ],
      }),
      flag('-D', '动态端口转发', {
        takesValue: true,
        args: [
          argument('forward', '动态代理监听地址', { suggestions: SSH_DYNAMIC_FORWARD_SUGGESTIONS }),
        ],
      }),
  ...
  command('curl', '发起 HTTP 请求', {
    flags: [
      ...
      flag('-u', '设置 HTTP 认证信息', {
        takesValue: true,
        args: [
          argument('credential', 'HTTP Basic 认证凭据', { suggestions: CURL_AUTH_SUGGESTIONS }),
        ],
      }),
      flag('-k', '忽略 TLS 证书校验'),
  ...
  command('journalctl', '查看 systemd 日志', {
    flags: [
      ...
      flag('-o', '指定日志输出格式', {
        ...
        takesValue: true,
        args: [
          argument('format', 'journalctl 输出格式', { suggestions: JOURNAL_OUTPUT_SUGGESTIONS }),
        ],
      }),
      flag('--since', '设置起始时间', {
        takesValue: true,
  ...
  command('awk', '按字段处理文本流', {
    flags: [
      flag('-F', '指定字段分隔符', {
        takesValue: true,
        args: [
          argument('separator', '字段分隔符', { suggestions: AWK_FIELD_SEPARATOR_SUGGESTIONS }),
        ],
      }),
      flag('-v', '设置变量', {
        takesValue: true,
        args: [
          argument('assignment', '变量赋值表达式', {
            suggestions: AWK_VARIABLE_ASSIGNMENT_SUGGESTIONS,
          }),
        ],
      }),
  ...
  command('yarn', 'Yarn 包管理器', {
    ...
    subcommands: [
      ...
      command('workspaces', '管理多工作区', {
        subcommands: [command('list', '列出工作区'), command('foreach', '遍历执行工作区脚本')],
      }),
      ...
      command('build', '构建项目', {
        flags: [
          ...
          flag('--target', '指定编译目标', {
            takesValue: true,
            args: [
              argument('target', 'Rust 目标三元组', { suggestions: CARGO_TARGET_SUGGESTIONS }),
            ],
          }),
      ...
      command('tree', '查看依赖树', {
        flags: [
          flag('-e', '指定边类型', {
            takesValue: true,
            args: [
              argument('edge', 'Cargo 依赖边类型', { suggestions: CARGO_TREE_EDGE_SUGGESTIONS }),
            ],
          }),
        ],
      }),
  ...
  command('python', 'Python 解释器', {
    flags: [
      ...
      flag('-W', '设置 warning 过滤规则', {
        takesValue: true,
        args: [
          argument('action', 'Python warning 处理策略', {
            suggestions: PYTHON_WARNING_SUGGESTIONS,
          }),
        ],
      }),
    ],
      ...
      command('list', '列出已安装包', {
        flags: [
          ...
          flag('--format', '指定输出格式', {
            takesValue: true,
            args: [
              argument('format', 'pip 列表输出格式', { suggestions: PIP_LIST_FORMAT_SUGGESTIONS }),
            ],
          }),
        ],
      }),
  ...
  command('helm', 'Kubernetes 应用包管理器', {
    ...
    subcommands: [
      ...
      command('list', '列出已安装 Release', {
        flags: [flag('-A', '跨全部命名空间显示'), flag('--all-namespaces', '跨全部命名空间显示')],
      }),
      ...
      command('search', '搜索 chart', {
        subcommands: [command('repo', '在仓库中搜索 chart'), command('hub', '在 Hub 中搜索 chart')],
      }),
